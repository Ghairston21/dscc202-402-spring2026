...
# MAGIC %fs ls dbfs:/databricks-datasets/samples
...
SELECT ROUND(AVG(totalPrice), 2) avg_transaction_value
FROM samples.bakehouse.sales_transactions
...
transactions_with_customers_df = (transactions_df
    .join(
        customers_df,
          # Join condition here
          on = transactions_df["customerID"] == customers_df["customerID"],
...
full_transaction_df = (transactions_with_customers_df
    .join(
        ...
        on = transactions_with_customers_df["franchiseID"] == franchises_df["franchiseID"],
        how = "inner"
    )
    .select(
...
# couldn't get count to work in .agg, so needed to explicitly call pyspark count
...
franchise_metrics_df = (transactions_df
    ...
    .agg(f.sum("totalPrice").alias("total_revenue"),
         ...
         f.avg("totalPrice").alias("avg_revenue"))
    .orderBy(f.desc("total_revenue")).limit(5)
)

day_metrics_df = (transactions_df
    .withColumn("day_name", date_format(col("dateTime"), "EEEE"))
    .groupBy("day_name")
...
payment_metrics_df = (transactions_df
